...
export default function GeschiedenisScreen() {
  ...
  const renderItem = ({ item }: { item: RitResultaat }) => {
    ...
    const extraTotaal = item.extraKosten.reduce((s, e) => s + e.bedrag, 0);

    return (
      <View
        style={[
          styles.ritItem,
          { backgroundColor: colors.card, borderColor: isOpen ? colors.primary : colors.border },
        ]}
      >
        {/* Hoofd-rij — klikbaar voor accordion */}
        <TouchableOpacity
          onPress={() => toggleAccordion(item.id)}
          activeOpacity={0.75}
          style={styles.ritHoofd}
        >
          <View style={styles.ritInfo}>
            ...
            <View
              style={[styles.routeLijn, { backgroundColor: colors.border }]}
            />
            <View style={styles.routeRij}>
        ...
        {/* Snelle meta-info altijd zichtbaar */}
        ...
        {/* Accordion detail-sectie */}
        {isOpen && (
          <View>
            <View style={[styles.accordionDivider, { backgroundColor: colors.border }]} />

            {/* Volledige prijsopbouw */}
            <View style={styles.prijsDetail}>
              <Text style={[styles.detailKop, { color: colors.mutedForeground }]}>
                Prijsopbouw
              </Text>
              <View style={styles.prijsRegel}>
                ...
                <Text style={[styles.prijsWaarde, { color: colors.foreground }]}>
                  {formatEuro(item.startTarief)}
                </Text>
              </View>
              <View style={styles.prijsRegel}>
                <Text style={[styles.prijsLabel, { color: colors.foreground }]}>
                  {item.afstandKm.toFixed(1)} km \u00D7 {formatEuro(item.kmTarief)}
                </Text>
                <Text style={[styles.prijsWaarde, { color: colors.foreground }]}>
                  {formatEuro(item.afstandKm * item.kmTarief)}
                </Text>
              </View>
              <View style={styles.prijsRegel}>
                <Text style={[styles.prijsLabel, { color: colors.foreground }]}>
                  {Math.round(item.tijdMin)} min \u00D7 {formatEuro(item.minTarief)}
                </Text>
                <Text style={[styles.prijsWaarde, { color: colors.foreground }]}>
                  {formatEuro(item.tijdMin * item.minTarief)}
                </Text>
              </View>
              {item.extraKosten.map((ek, idx) => (
                <View style={styles.prijsRegel} key={idx}>
                  <Text style={[styles.prijsLabel, { color: colors.warning }]}>
                    {ek.beschrijving}
                  </Text>
                  <Text style={[styles.prijsWaarde, { color: colors.warning }]}>
            ...
            {/* Datum volledig */}
            <Text style={[styles.volledigeDatum, { color: colors.mutedForeground }]}>
            ...
            <View style={styles.actiesRij}>
              ...
              >
                ...
                <Text style={[styles.actieTekst, { color: colors.primary }]}>
                  Delen
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => verwijderRit(item)}
                activeOpacity={0.7}
                style={[
                  styles.actieKnop,
                  {
              ...
              >
                ...
                <Text style={[styles.actieTekst, { color: colors.destructive }]}>
                  Verwijderen
                </Text>
              </TouchableOpacity>
            </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.leegState}>
            <View
              style={[styles.leegIconWrapper, { backgroundColor: colors.card }]}
            >
